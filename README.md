...
3. **Add GitHub secrets**:
    ...
    | `TF_BUCKET_PREFIX` | Bucket prefix/path (e.g., `gcp-iam-rbac-google-groups`) |

4. **Run the workflow**:
    - **Apply**: Go to Actions → **CD - GCP IAM RBAC Google Groups (Apply)** → fill in all inputs
    - **Destroy**: Go to Actions → **CD - GCP IAM RBAC Google Groups (Destroy)** → fill in essential inputs
...
terraform {
  backend "gcs" {
    ...
    prefix = "gcp-iam-rbac-google-groups"
  }
}
```
